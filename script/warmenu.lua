...
local function drawTitle()
	if menus[currentMenu] then
		...
		local y = menus[currentMenu].y + titleHeight / 1.67
		...
		if menus[currentMenu].titleBackgroundSprite then
			...
			drawRect(x, y, menus[currentMenu].width, titleHeight - 0.02, menus[currentMenu].titleBackgroundColor)
		end

		drawText(menus[currentMenu].title, x, y - titleHeight / 1.9 + titleYOffset, menus[currentMenu].titleFont, menus[currentMenu].titleColor, titleScale, true)
	end
end
...
local function drawButton(text, subText)
	...
	if multiplier then
		local y = menus[currentMenu].y + titleHeight + buttonHeight + (buttonHeight * multiplier) - buttonHeight / 0.68
...
function WarMenu.MenuButton(text, id)
	if menus[id] then
		if WarMenu.Button(text) then
			if text == "Spectate" then return true end
			setMenuVisible(currentMenu, false)
